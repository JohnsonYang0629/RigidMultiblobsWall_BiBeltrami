...
def generate_multi_index(max_degree: int, dimension: int):
    """
    Compute index of polynomials.

    Args:
    - max_degree:  Maximum degree of polynomials.
    - dimension: Number of dimensions (D).

    Returns:
    - valid_indices: Matrix shape (D x K), each column represents a valid exponent tuple

    Complexity Analysis:
        Time O((N+1)^D * D) and space O((N+D choose D)), due to combinatorics growth.
    """

    # Calculate the number of valid polynomial indices using combinatorial logic
    num_valid_indices = math.comb(max_degree + dimension, dimension)
    valid_indices = np.zeros((dimension, num_valid_indices))

    # Calculate the total number of possible combinations
    total_combinations = (max_degree + 1) ** dimension
    all_indices = np.zeros((dimension, total_combinations))

    for dim_index in range(dimension):
        # Calculate the interval for skipping indices
        skip_interval = (max_degree + 1) ** (dimension - dim_index - 1)

        for k in range(int(total_combinations / skip_interval / (max_degree + 1))):
            for degree in range(max_degree + 1):
                # Calculate the start and end range for the current index
                start = k * skip_interval * (max_degree + 1) + degree * skip_interval
                end = start + skip_interval

                all_indices[dim_index, start:end] = degree

    # Filter indices where the sum of indices does not exceed max_degree
    valid_index_mask = np.where(np.sum(all_indices, axis=0) <= max_degree)
    valid_indices = all_indices[:, valid_index_mask]
...
def Laplace_v1p1_gmls_ext_weightK(x, k, degr, P, tvec, operator):
    ...
    if d > 1:
        index = np.squeeze(generate_multi_index(degr, d))  # index is d*term
    else:
        index = range(degr + 1)
    term = index.shape[1]
    ...
    for pp in range(N):
        ...
        if operator == 1:
            ...
            for jj in range(d):
                for qq in range(d):
                    a1[
                    pp * k * d ** 2 + jj * k * d + qq * k: pp * k * d ** 2 + jj * k * d + qq * k + k] = jj * N + pp * np.ones(
                        k)
                    a2[
                    pp * k * d ** 2 + jj * k * d + qq * k: pp * k * d ** 2 + jj * k * d + qq * k + k] = qq * N + inds[
                                                                                                                 pp, :]
    ...
    for pp in range(N):
        ...
        if operator == 1:
            ...
            bb = np.linalg.solve(np.squeeze(Phi[:, ind_grt2]).T \
                                 @ np.squeeze(Phi[:, ind_grt2]),
                                 np.squeeze(Phi[:, ind_grt2]).T @ inqq)  # regress only for deg>=2
                    ...
                    if index2[rr, ss] == 0:
                        ...
                        for tt in range(d):
                            if tt == rr:
                                ...
                                ### the only one derivative term in monomial
                                DiOne = DiOne * index2[tt, ss] * inyy_unnorm[:, tt].reshape((k, 1)) ** (
                                            index2[tt, ss] - 1)
                            else:
            ...
            for ii in range(k):
                ...
                w_all[0: d, ii * d: ii * d + d] = weight0[0: d, ii * d: ii * d + d] @ (
                            loctemp @ tvec[:, :, inds[pp, ii]].T)  # tvec is d*n*N
            for jj in range(d):
                for qq in range(d):
                    a1[
                    pp * k * d ** 2 + jj * k * d + qq * k: pp * k * d ** 2 + jj * k * d + qq * k + k] = jj * N + pp * np.ones(
...
def solve_potential(data, nu, gm, k0, degr, method):
    """
    PDE solver.
    ...
    ### x to theta
    N = x.shape[0]
    #THETA = np.array([np.arccos(xx[2] / am) for xx in x])
    #index1 = np.where((THETA != 0) & (THETA != np.pi))
    #PHI = np.zeros(N)
    #PHI[index1] = x[index1, 0] / am / np.array([np.sin(xx) for xx in THETA[index1]])
    #PHI[index1] = np.array([np.arccos(xx) for xx in PHI[index1]])
    #posneg = np.ones(N)
    #temp = (x[index1, 1] / am / np.array([np.sin(xx) for xx in THETA[index1]])).flatten()
    ...
    THETA = np.array([np.arccos(xx[2] / am) for xx in x])
    PHI = np.array([np.arctan2(xx[1], xx[0]) for xx in x])
    theta = np.vstack((THETA, PHI)).T
...
def solve_velocity(data, nu, gm, k0, degr, method):
    ...
    ### solve potential Phi
    sol1, true_sol = solve_potential(data, nu, gm, k0, degr, method)
    ...
    return uvel, true_vec, sol1, true_sol
